"""등급 접근 제어 제출 증거 화면을 PC 해상도로 자동 캡처한다.
    ...
    python scripts/capture_role_evidence.py

images/role-access/ 아래에 01~15번 PNG를 만든다. 1440x900 뷰포트에 2배
스케일이라 글자가 또렷하다.

캡처 전에 데모 계정과 게시글을 심고, 끝나면 normal01을 다시 일반(0)으로
되돌린다. 그래서 몇 번을 돌려도 같은 결과가 나온다.
"""
import sys
...
from models import ROLE_ADMIN, ROLE_GOLD, ROLE_USER, Post, User  # noqa: E402
...
def restore():
    """캡처 도중 바뀐 등급과 삭제된 게시글을 처음 상태로 되돌린다."""
    seed()
...
def capture():
    with sync_playwright() as pw:
        ...
        shooter.visit(page, "/")
        # 관리자 홈에는 남의 글마다 '관리자 삭제' 버튼이 함께 찍힌다.
